...
export function MyAccount() {
  ...
  const nftsSource = {
    weapons:
      connected && metadata && metadata.length > 0
        ? metadata.filter((nft) =>
            nft.Category && nft.Category.toLowerCase().includes("weapon")
          )
        : [],
    pets:
      connected && metadata && metadata.length > 0
        ? metadata.filter((nft) =>
            nft.Category && nft.Category.toLowerCase().includes("pet")
          )
        : [] // hoặc nếu có nguồn NFT của Pets khác
  };
  useEffect(() => {
            ...
            {currentItems.map((nft: any) => (
              ...
              >
                <div className="relative h-40 bg-gray-700">
                  <Image
                    src={nft.image ? `https://gateway.pinata.cloud/ipfs/${nft.image}` : "/placeholder.svg"}
